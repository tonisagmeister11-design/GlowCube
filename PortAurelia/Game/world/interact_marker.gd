class_name InteractMarker
extends Node3D
## Glowing ground marker the player can interact with (E). Used by shops, garages,
## safehouses, mission starts and bus stops. Vehicle markers (drive-in, e.g. the
## mechanic) are triggered by the owner system instead of the E key.

var prompt := "E: Interagieren"
var interact_radius := 2.2
var color := Color(1.0, 0.8, 0.2)
var on_interact: Callable
var condition: Callable
var vehicle_marker := false
var show_visual := true
var _ring: MeshInstance3D

static var _mat_cache := {}


func _ready() -> void:
	if not vehicle_marker:
		add_to_group("interactable")
	if show_visual:
		_build_visual()


func _build_visual() -> void:
	var r := interact_radius * (0.42 if not vehicle_marker else 0.8)
	var h := 0.7 if not vehicle_marker else 1.2
	var key := "%s_%.2f" % [color.to_html(), h]
	var mat: ShaderMaterial = _mat_cache.get(key)
	if mat == null:
		mat = ShaderMaterial.new()
		mat.shader = preload("res://assets/shaders/marker.gdshader")
		mat.set_shader_parameter("color", Color(color.r, color.g, color.b, 0.6))
		mat.set_shader_parameter("height", h)
		_mat_cache[key] = mat
	_ring = MeshInstance3D.new()
	var cyl := CylinderMesh.new()
	cyl.top_radius = r
	cyl.bottom_radius = r
	cyl.height = h
	cyl.cap_top = false
	cyl.cap_bottom = false
	cyl.radial_segments = 32
	cyl.rings = 1
	cyl.material = mat
	_ring.mesh = cyl
	_ring.position.y = h * 0.5
	_ring.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
	_ring.visibility_range_end = 150.0
	add_child(_ring)


func can_interact(p: Node) -> bool:
	return not condition.is_valid() or bool(condition.call(p))


func get_prompt(_p: Node) -> String:
	return prompt


func interact(p: Node) -> void:
	if on_interact.is_valid():
		on_interact.call(p)
