...
extends Menu {
    ...
    protected void draw() {
        ...
        this.set(33, Ui.icon(Material.BELL, "<gold><bold>Ansage schreiben</bold>", List.of("<gray>Schickt eine Nachricht an alle", "<gray>Spieler auf dem Server.", "", "<yellow>➤ Klicken, dann in den Chat tippen")), inventoryClickEvent -> this.plugin.state().prompt(this.viewer, "Was soll allen angesagt werden?", string -> {
            Component component = Ui.mm("<gradient:#5ad1ff:#a06bff><bold>Ansage</bold></gradient> <dark_gray>»</dark_gray> <white>" + string);
            Bukkit.broadcast((Component)component);
            this.plugin.log().add(ActivityLog.Level.INFO, this.viewer.getName() + " sagte an: " + string);
        }));
