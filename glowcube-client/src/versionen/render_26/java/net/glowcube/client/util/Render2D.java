...
/**
 * Fassung fuer <b>26.3 und neuer</b>. Wortgleich zur 1.21-Fassung, nur auf
 * Mojangs neuem 2D-Typ: {@code GuiGraphics} heisst ab 26.x
 * {@code GuiGraphicsExtractor}, und der Textbefehl heisst {@code text} statt
 * {@code drawString}. {@code fill}/{@code fillGradient}/{@code enableScissor}
 * sind unveraendert - genau darum ist diese Schicht so schmal.
 */
public final class Render2D {
    ...
    public static void gradientV(GuiGraphicsExtractor gfx, float x, float y, float w, float h, int top, int bottom) {
    ...
    public static void roundedRect(GuiGraphicsExtractor gfx, float x, float y, float w, float h, float radius, int color) {
        int xi = Math.round(x);
        int yi = Math.round(y);
        int wi = Math.round(w);
        int hi = Math.round(h);
    ...
    public static void roundedGradientH(GuiGraphicsExtractor gfx, float x, float y, float w, float h,
    ...
    public static void roundedOutline(GuiGraphicsExtractor gfx, float x, float y, float w, float h, float radius, int color) {
        int xi = Math.round(x);
        int yi = Math.round(y);
        int wi = Math.round(w);
        int hi = Math.round(h);
    ...
    public static void glow(GuiGraphicsExtractor gfx, float x, float y, float w, float h, float radius, int color, int layers) {
    ...
    public static void text(GuiGraphicsExtractor gfx, String text, float x, float y, int color) {
        gfx.text(font(), text, Math.round(x), Math.round(y), color, false);
    }

    public static void textShadow(GuiGraphicsExtractor gfx, String text, float x, float y, int color) {
        gfx.text(font(), text, Math.round(x), Math.round(y), color, true);
    }

    public static void textCentered(GuiGraphicsExtractor gfx, String text, float centerX, float y, int color) {
        text(gfx, text, centerX - width(text) / 2.0f, y, color);
    }

    /** Buchstabe fuer Buchstabe durch einen Verlauf gefaerbt. */
    public static void textGradient(GuiGraphicsExtractor gfx, String text, float x, float y, int from, int to) {
        float cursor = x;
    ...
    public static void pushScissor(GuiGraphicsExtractor gfx, float x, float y, float w, float h) {
